...
function go() {
	...
	if ( searchBox ) {
		const eventuallyUpdateQuery = debounce( onQuery, 400 );
		...
		cards = Array.prototype.reduce.call(
			...
			( all, category ) => {
				category.categoryName =
					category.categoryName ||
					category.querySelector( '.category-title' ).innerText;
				const catCards = category.querySelectorAll( '.a-card' );
...
function createResult( data ) {
	...
	const eventLocationEl = clone.querySelector( '.card-event-location' );
